...
class HarkoniansAPI {
    ...
    static async receiveItem(itemData) {
        return this.request('/api/foundry/items/receive', {
            method: 'POST',
...
class HarkoniansWebSocket {
    ...
    static connect() {
        ...
        const url = `wss://api.harkonians.quest/v1/foundry/ws?token=${encodeURIComponent(token)}`;
    ...
    static handleMessage(data) {
        switch (data.type) {
            case 'purchase':
                ...
                HarkoniansPurchaseHandler.handlePurchase(data.purchase);
                break;
            case 'gold_update':
...
Hooks.once('init', () => {
    ...
    Hooks.on('updateActor', async (actor, changes, options, userId) => {
        ...
        if (goldChanged) {
            // Sync gold to Harkonians if actor is linked
